...
## Merge 10 — Card 7 `sync-doc-honesty` → `overnight-20260901`

- **Merge commit:** `29793e9` (`--no-ff`). **Card branch:** `wo-sync-doc-honesty` (tip `646cb81`).
- **Cards involved:** Card 7 (Track B, 4th). Run branch had advanced (Cards 1–6,9,10,11).
- **Files / hunks:** 10 files, +311/−46: `workflows.html` (2 comments retired — precached), `sw.js` (regen, workflows.html + conflict-notice-ui.js revisions), `backend/internal/sync/proxy.go` (comment-only fictions), `sync-rxdb/conflict-notice-ui.js`, `sync-schema/sql/0001_sync_tables.sql` (5th gate), `tests/repo-hygiene.spec.js` (case 3 broadened), `tests/states-sync-rxdb-conflict-notice.spec.js`, `roadmap.md` (DONE), merge-intent.
- **Shared surfaces:** `sw.js` — Card 11 changed build-sw.js NOT sw.js; no conflict. `proxy.go` — Card 5 touched ops.go NOT proxy.go; no conflict. `roadmap.md` — own section. **No conflict.**
- **Conflict?** **NONE — clean** (ort).
- **G6 verdict:** **PASS** (a6007f5f) — independent whole-tree grep: **0 LIVE stale gates** (all hits retrospective/frozen); proxy.go comments factually true + comment-only (zero non-comment lines); B-167's 6 items re-homed (none dropped); repo-hygiene case 3 broadened to fact-matching, red-first proven (paraphrased DONE-card gate caught) + false-positive-checked (retrospective quotes NOT flagged); precache 31, workbox chunk preserved; confined Playwright subset **208 passed / 1 failed** (B-176 baseline, no new red). nice-to-have: merge-intent prose overstates B-18(b) (says logging added; actually correctly deferred as a comment — engineering decision sound, prose nit only).
- **Gate after merge (control-loop re-gate on `29793e9`):** G1 `go build ./...` 0, `go vet ./...` 0. **G4 (sw.js reconciliation — the slate's post-Cards-7&11 regen point):** `node build-sw.js` on the merged tree → **31 files precached, ZERO working-tree delta** — Card 7's committed sw.js is exactly reproducible (idempotent); no workbox rehash to discard this pass. Version parity 1.5.0. Playwright footprint = G6's 208/1 subset (authoritative). `internal/sync` red unchanged = B-178.
- **Roadmap:** `sync-doc-honesty` DONE. **sw.js reconciliation COMPLETE — no further regen needed at closeout (Card 8 touches no precached asset).**

## Merge 11 — Card 8 `app-slug-association` → `overnight-20260901` (FINAL MERGE — 11/11)

- **Merge commit:** `809e15d` (`--no-ff`). **Card branch:** `wo-app-slug-association` (tip `44c1485`).
- **Cards involved:** Card 8 (Track B, LAST — completes the slate). Run branch had all 10 prior merges.
- **Files / hunks:** 6 files, +391/−22: `backend/internal/sync/spikec_relay.go` (app_slug now per-row from association), `backend/cmd/spikec-relay/main.go` (drops SPIKE_C_APP_SLUG read), NEW `backend/internal/db/migrations/0076_checklist_template_app.sql`, NEW `app_slug_association_test.go`, `roadmap.md` (DONE), merge-intent (with the E-KR4 home decision).
- **Shared surface:** `internal/sync/*` — Card 5 added symbols to `ops.go`, Card 7 fixed comments in `proxy.go`; Card 8 touches `spikec_relay.go` — **all disjoint files. No conflict.** `roadmap.md` — own section.
- **Conflict?** **NONE — clean** (ort).
- **G6 verdict:** **PASS** (af1c6bd8). Migration 0076 sound (nullable FK `checklist_templates.app_id → hq_apps`, indexed, valid Down, backfill all templates→operations reproduces today's value, bare-DB NULL-safe via COALESCE). **B-160 GENUINELY DISCHARGED (load-bearing completeness check):** FDW views 0073/0074 project NO app_slug (read-through RLS per decision 92), so `spikec_relay.go` is the ONLY projection writer; 0 hardcoded constants remain (independent grep). Red-first genuine (structural + per-template-distinct-resolution tests). Only `internal/sync` red is B-178 `TestJWTBridgeRLS` — and the leftover spikec rows carry a **correctly-resolved** `app_slug:"operations"`, further proving the resolver. Association-home (1:N child FK) is right, NOT a product fork; PARK correctly not taken. 2 pre-existing nice-to-haves noted (internal/sync has no TestMain; access_test.go TRUNCATE ops coupling).
- **E-KR4 decision:** `checklist_templates.app_id UUID REFERENCES hq_apps(id) ON DELETE SET NULL` (store FK id, resolve → slug at projection). Recorded in the merge-intent.
- **Gate after merge (control-loop re-gate on `809e15d`):** G1 `go build ./...` 0, `go vet ./...` 0. Full-tree gate runs at closeout (see closeout record). `internal/sync` red = B-178 (environmental).
- **Roadmap:** `app-slug-association` DONE. **11/11 cards landed.**
