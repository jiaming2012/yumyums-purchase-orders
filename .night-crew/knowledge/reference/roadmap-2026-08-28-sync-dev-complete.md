...
- **`gate-rls-fixture-ownership`** · **DONE** (attended re-gate 2026-08-28, branch `card/a3-rls-fixture-own-regate`, ledger §T-45 addendum) · Closed **B-35**, **B-141** (guard half — entry now fully closed), **B-142**. The preserved `card/a3-rls-fixture-own` branch was lost to a worktree sweep; re-implemented from the recorded leads. Required prefix `^hq_rls_[a-z0-9_]+$` examined before any socket (incident probe `HQ_RLS_TEST_DB=yumyums` pinned refused by a subprocess test run RED first); per-process default fixture name (B-35's shared constant gone, no Taskfile env needed); claim-don't-drop (an existing database is refused, EXIT≠0, provably not destroyed). Gate: full Go suite green (11 packages) against an isolated substrate; the persistent dev substrate reds `TestJWTBridgeRLS` with `spikec-*` residue on ANY tree — B-50 aggravation filed, not this card's regression. Original G6 context: its probe destroyed production (B-141/B-143, decision 155).
...
- **`dev-complete-attestation`** · **DONE** (attended, 2026-08-28 — the operator ran the
  app-surface attestation and recorded it at ledger T-45; milestone closed the same sitting,
  T-45 close line, marker `hq-20260828`. Status flipped at the 2026-08-28 roadmap round, which
  found the card still reading PLANNED after the close) · 🛑 **Attended, and the operator's own act.** Per
