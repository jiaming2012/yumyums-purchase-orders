...
- **B-39 · The same `'submitted'`/`'completed'` doc rot ledger decision 66 filed for `tests/sync.spec.js:1584` ALSO sits at `tests/repro-cut-task.spec.js:169`, and no obligation names it** — obligation 8 of `sync-cache-and-identity-hygiene` was written against one line anchor, so the fix landed on one line. `tests/repro-cut-task.spec.js:169` carries the identical claim (`requires_approval false → 'submitted', freezes the snapshot`) and the server sets `'completed'` (`backend/internal/workflow/repository.go:715-716`). Not folded in under the standing scope freeze: it reddens no key result, and `tests/repro-cut-task.spec.js` is outside B1's footprint and outside the card's obligations. · _run `overnight-20260802`, card B1, found while discharging obligation 8_ · new · lead: one word, and the same one. The reusable lesson is the shape, not the line: **an obligation written as a file:line anchor fixes an instance, not a class** — decision 66 recorded "the stale comment at `tests/sync.spec.js:1584`" when what it had found was a claim about submit status that the codebase repeats. A grep-defined obligation (`grep -rn "requires_approval false" tests/`) would have caught both and cost nothing more. Sibling of B-25 in kind (line anchors rot); this is the version where the anchor was not merely stale but too narrow from the day it was written.

- **B-40 · `users.spec.js:1185` (FR-18/19 grant chip remove) times out at 30s inside the full suite and passes in 1.7s alone — and the suite's single failure MOVED between two runs of the same tree** — two full `npx playwright test --retries=0` runs of the identical worktree (`overnight-20260802`, card B1, isolated stack `TEST_PORT=8305` / `hq_n802_b1i_e2e`, DB dropped and recreated before each) produced **one failure each, and not the same one**. Run 1: `list page progress decrements when another device unchecks a field [LST-17]` (armed, expected). Run 2: `Users UI + Access hardening › FR-18/19: adding then removing an individual user grant persists across reload` — `page.waitForResponse` for `PUT /api/v1/apps/{slug}/permissions` never resolved, 30.4s timeout. Re-run in isolation immediately afterwards: **passes in 1.7s**. Each run's other failure passed. 🛑 **The point is not either test.** It is that a 743-test serial suite on one shared server + one shared database has a ~1-failure-per-run floor drawn from a rotating pool, which means **every card's G2 verdict is judged against a baseline that is not stable between two runs of the same commit** — and the ritual's own rule (judge against the B-27 baseline, name every survivor by full title) assumes it is. B-27 itself (`tests/inventory.spec.js:883`) **passed in BOTH runs**, so the documented baseline red did not reproduce either. · _run `overnight-20260802`, card B1, G2 leg; found by re-running the full suite after the first run's log ended in a hung teardown_ · new · lead: **do not chase either test first.** Establish whether the floor is real: run the suite N=3 on an untouched `dev` and record which titles fail each time. If the pool is real, the actionable fix is the config comment's own escape hatch — `PW_WORKERS` with per-worker `TEST_DB_NAME`+`TEST_PORT` — because both observed failures are `waitForResponse`/`toContainText` timeouts under contention on the single `go run` server, not assertion mismatches. Sibling of B-27 in kind (a red the baseline names) but larger: this says the baseline is a distribution, not a list. Related: the first run's Playwright process **hung after printing its summary** (webServer child not reaped; it held :8305 for 35 minutes until killed by hand), and that run's failure stack frame printed a path from **another card's worktree** (`a1-sync-replication-scope-per-checklist`) while the clean re-run printed the correct one — worth confirming the shared `/tmp/playwright-transform-cache-1000` is not serving cross-worktree sourcemaps.
