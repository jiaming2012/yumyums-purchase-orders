...
# GO (browser) — RxDB reaches a no-build-step browser as a committed vendored bundle, persists to Dexie across reloads, elects a leader across tabs, and recovers from token expiry. One hard constraint falls out: **the replication endpoint must not be mounted under `/api/`.**

**Card:** `sync-rxdb-browser-delivery-spike` (night-crew run `overnight-20260726`,
2026-07-26).
**Runbook:** [`.night-crew/qa/spike-supabase/README.md`](../../qa/spike-supabase/README.md),
**half 3**.
**Harness:** `.night-crew/qa/spike-supabase/browser/` (11 Playwright tests, all
green) and `vendor/` (generator + committed bundle).

**Red-first: N/A — stated, not omitted.** QA KR2's red-first requirement does not
apply to a card whose deliverable is a **verdict**. There is no production
behaviour to hold red: the output is a GO/NO-GO finding, a runbook, and a
delivery mechanism that **no shipped page imports**. This is recorded here, in
the merge-intent note, and on the roadmap card, so that a considered "N/A" and an
empty field cannot be mistaken for one another. *(Two legs did in fact begin red
and that is how their findings were made — leg 1 on `rxdb.info`, leg 5 on the
PostgREST skew leeway — but that is incidental, not the KR's red-first
discipline.)*

## The verdict in one paragraph

**GO.** All five things a Node-side proof could not establish were established in
a real Chromium against W1's stack. RxDB reaches the browser as a **committed,
vendored ESM bundle** (`vendor/rxdb.bundle.js`, 506,885 B raw / 148,591 B gzip,
generated by a hand-run `npx esbuild@0.28.1` and loaded with a plain
`<script type="module">`), which keeps HQ's "no build step in the deploy path"
constraint intact and makes the offline data engine a Workbox-precached local
asset — **verified to load with the network cut.** Dexie/IndexedDB survives a
real page reload with the documents intact. HQ's Workbox service worker does
**not** interfere with replication and does **not** touch WebSockets. Multi-tab
leader election works at the browser default (`waitForLeadership: true`, which
W2 had to disable): exactly one tab replicates, and the survivor takes over and
**begins replicating** within ~50 ms of the leader closing. A token that expires
during an offline period costs nothing: replication retries, the offline write
sits safely in Dexie, and recovery is a **token swap on the existing client** —
no re-login, no database teardown, no data loss. **Two findings constrain the
next card and one reshapes the bundle**, below.

## 🛑 The hard constraint: do not mount Supabase under `/api/`

HQ's Workbox `sw.js` registers one runtime route: `NetworkFirst` on `/\/api\//`
with an offline JSON fallback. `NetworkFirst` tries **network → cache →
`handlerDidError`**. The feasibility note anticipated the third of those. **The
second is the dangerous one and it is real.**

Measured (runbook half 3, leg 3c): with the service worker controlling and the
network cut, a same-origin, replication-shaped pull under `/api/` was answered
with **HTTP 200, `application/json`, a well-formed array of rows** — stale,
missing a row inserted out of band, and **indistinguishable from a fresh pull.**
RxDB would accept those documents and **advance its `{modified, id}` checkpoint
past data it never saw.** The row would never be pulled again. The identical
query **not** under `/api/` failed honestly with `TypeError: Failed to fetch`,
which RxDB retries correctly.

> **Constraint for `sync-rxdb-schema-and-replication` and `sync-hard-cutover`:**
> mount the Supabase/PostgREST endpoint at a path the Workbox route does not
> claim — `/rest/v1/*`, `/sync/*`, or cross-origin. If it ever must live under
> `/api/`, `build-sw.js` must exclude that sub-path from the runtime route
> **explicitly**, and that exclusion needs a regression test. **The failure mode
> is silent data loss, not an error.**

Corollary worth carrying: the **WebSocket half fails loudly** where the HTTP half
fails silently. A service worker cannot answer a WS handshake, so Realtime can
never be served stale from `api-cache`.

## The second finding: RxDB's dev-mode plugin phones home, on any host

`addRxPlugin(RxDBDevModePlugin)` appends a hidden 1×1 iframe pointing at
`https://rxdb.info/html/dev-mode-iframe.html` to `document.body` and hashes
`location.host`. Its own guard comment says "Only run this in browser AND
localhost AND dev-mode. Make sure this is never used in production by someone" —
and **`!isLocalHost()` is commented out in the shipped 17.4.0 build**
(`plugins/dev-mode/dev-mode-tracking.js:36-44`). Only a paid premium flag
suppresses it.

Found because leg 1 asserts the page reaches **zero external hosts** and went red
on `rxdb.info`. dev-mode and its mandatory `validate-ajv` wrapper were removed
from the vendored entry: −201,671 B raw, −57,174 B gzip, and zero third-party
hosts. Knock-ons: W2's sharp edge 9 (DVM1) stops applying, and
**`ignoreDuplicate: true` becomes illegal** (`RxError DB9`) — W2's `spike-env.js`
sets it and never hit this.

## The third finding: PostgREST tolerates ~30 s of `exp` skew

W1 proved `exp` is enforced. It is — **with about 30 seconds of leeway.** Fresh,
never-before-seen tokens (so not a JWT-cache artefact) against PostgREST
v12.2.12: `exp` − 5 s / − 15 s / − 25 s → **200**; − 29 s / − 30 s / − 31 s /
− 35 s / − 45 s / − 60 s → **401 PGRST301 "JWT expired"**.

This nearly produced a false green: the first version of the token-expiry leg
waited 28 s past `exp` and the write pushed successfully. **Any HQ refresh
design that treats `exp` as a hard edge is wrong by half a minute, and any test
that waits less than ~30 s past `exp` measures the leeway rather than the
expiry.**
...
1. **The client construction is settled on the REST side.** Give `createClient`
   ...
   **W2's `global.fetch` shim is not needed** when PostgREST is fronted on HQ's
   origin. FORK 4's "permanent client-construction helper" shrinks, on the REST
   side, to a base-URL choice plus a token source.
2. **The token source must be a mutable callback, not a baked-in string.**
   `supabase-js` calls the `accessToken` callback on **every** request
   (`dist/index.mjs:673`), which is exactly what makes expiry recoverable
   without tearing the database down. Design the helper around that.
3. **`waitForLeadership` should be left at its default (`true`).** W2's `false`
   was a Node-harness concession. Proven correct in tabs, with ~50 ms handover
   and the survivor genuinely resuming replication.
4. **Storage is `getRxStorageDexie()`, unwrapped.** No ajv wrapper, no dev-mode,
   no `ignoreDuplicate`.
5. **The bundle is `vendor/rxdb.bundle.js`**, precached via
   `build-sw.js`'s `vendor/*.bundle.js` glob. Regeneration is
   `bash vendor/build-vendor.sh` + `node build-sw.js`, hand-run on upgrade only.
   FORK 4's upgrade smoke test exists: `VENDOR_BUILD` is stamped from the
   lockfile and asserted against RxDB's own `RXDB_VERSION` in
   `specs/leg1-delivery.spec.js`.
6. **Errors must be classified, not just surfaced.** `jwt-expired` and `network`
   arrive through the same `error$` with the same `RC_PUSH` code. "You are
   offline" and "please sign in again" are not interchangeable messages to a
   crew member on a truck. This is an input to `sync-rxdb-conflict-notice-ui`
   as much as to this card.
7. **Do not mount Supabase under `/api/`.** See above. This one is a hard gate.

## What this card did NOT establish — stated plainly

- **Safari, and therefore iOS.** Chromium only. Playwright's WebKit is not
  Safari-on-iOS, and a green there would over-claim. **Quota limits, eviction
  under storage pressure, Safari's stricter eviction, and private browsing are
  all untested.** This is the largest remaining unknown for a phone-first PWA and
  it wants a real device, not a headless browser.
- **A browser reaching self-hosted Realtime directly.** A direct handshake to the
  `realtime-dev.localhost` vhost did not connect in Chromium. **Recorded, not
  asserted** — W1 already proved Realtime over a Go-minted token with `rtwatch`,
  and this is their substrate surface. Nothing in this card depends on it, but
  `sync-rxdb-schema-and-replication` must budget for it.
- **Realtime's token lifecycle.** `supabase-js` needs a separate `setAuth()` on
  the Realtime socket when the token changes. The **HTTP** half of replication
  recovers on a token swap; the **WebSocket** half is untested.
- **Adoption.** No production page imports the bundle. `workflows.html` is
  untouched. This card proves a delivery path; it does not adopt one.
- **A real build step.** Candidate 2 (esbuild in `task sw` / the Dockerfile)
  remains the cleaner long-term answer and remains an **operator decision**. The
  vendored bundle works, so nothing forces the question tonight.
- **HQ's real schema, volume or relations.** Still one flat fixture table with a
  single-owner predicate, exactly as W2 left it.
