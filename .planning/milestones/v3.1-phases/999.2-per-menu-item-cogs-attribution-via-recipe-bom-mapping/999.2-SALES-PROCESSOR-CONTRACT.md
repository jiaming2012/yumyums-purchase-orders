...
- **Query params:**
  ...
  - 🛑 **CHANGED 2026-08-01 — was `America/Chicago`.** This endpoint shares its date semantics with Phase 21's `/period-summary`, so it moves with it. The authoritative statement and the sequencing requirement live in **assumption A5 of `21-SALES-PROCESSOR-CONTRACT.md` §5** — it is a **coordinated two-repo release**, and until sales-processor ships its matching change the two repos disagree by one hour at each period edge. Read A5 before deploying either side.
...
- [ ] **A10 (ADDED 2026-08-01): the operating timezone is `America/New_York`, and changing it is a COORDINATED TWO-REPO RELEASE.** It was `America/Chicago`. This endpoint shares its `from` / `to` date semantics with Phase 21's `/period-summary`, so it does not get to hold an independent opinion about which day a receipt belongs to. **The authoritative statement, the sequencing requirement, and the bounded blast radius are all in assumption A5 of `21-SALES-PROCESSOR-CONTRACT.md` §5** — confirm that one, not this one. This entry exists so a reader who only ever opens the menu-cogs contract still learns that the zone moved.
