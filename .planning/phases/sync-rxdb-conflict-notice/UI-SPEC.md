...
done_when:
  ...
  - "18. EVERY interactive element in the design is a >=44px touch target in both dimensions — not just the two classes already known to pass. shoot.mjs measures getBoundingClientRect over '.cf-btn, .cg-all, .cf-done-undo, .sc-close, .cn-banner-go, .sc-err button, .sc-empty button, .cfm-go, .cfm-cancel' in both schemes, prints the count measured (62 after the repair round, 58 at r2, up from 38) and every element under 44px, and exits non-zero if the list is non-empty. Undo, Done and the banner's Review are inside that selector. Grepping the stylesheet does NOT satisfy this row — it was what let three sub-44px controls through"
