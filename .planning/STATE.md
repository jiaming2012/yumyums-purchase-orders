...
| 260630-mav | Inventory Purchases tab — red "Missing Receipt" badge for no-attachment pending purchases. Adds `.approval-badge.missing` CSS modifier (red palette, light + dark mode); `renderPendingCard()` branches on `p.reason==='no_attachment_on_bank_tx'` to render "Missing Receipt" instead of "Needs Review" and suppress the raw sentinel string from the meta row. Other reasons (parse failed, totals mismatch) keep the existing amber "Needs Review" badge and reason text. Local var renamed to `pendingBadgeHtml` to avoid shadowing the global `badgeHtml(level)` stock helper | 2026-06-30 | 0d7140f |  | [260630-mav-missing-receipt-badge](./quick/260630-mav-missing-receipt-badge/) |
| 260701-a23 | Rewrite Purchases-tab sync chip copy so "13 processed, 13 skipped" stops conflating "Mercury txns examined" with "purchase_events created." Backend adds `lookback_days` int to `/sync-receipts/status` JSON (`SyncReceiptsStatusHandler` signature now `(pool, lookbackDays)`; wired from `receiptCfg.LookbackDays`). Frontend `renderSyncUI` branches on `s.triggered_by`: manual → `Synced from {Mon DD} — N checked[, M auto-added][, K pending review][, C skipped]` where `{Mon DD}` = `started_at − lookback_days days`; reprocess_all → `Reprocessed N pending — X auto-added, Y still pending[, Z errored]` (Z reads `s.cached`, which stores errored count on reprocess runs per reprocess_pending.go:184-189 semantic overload, noted in inline comment). Graceful fallback to legacy "Last synced X" copy when `lookback_days` missing/0 so old-backend + new-frontend combo never renders `undefined`. Failed-run chip unchanged. 2 new Playwright tests + 1 new backend JSON-shape assertion. Both semvers bump (backend 0.1.0→0.1.1, frontend 1.0.0→1.0.1). | 2026-07-01 | 7926b4a |  | [260701-a23-rewrite-the-purchases-tab-sync-chip-copy](./quick/260701-a23-rewrite-the-purchases-tab-sync-chip-copy/) |
| 260702-l67 | Retry parse click auto-triggers the receipt sync — piggybacks on existing SYNC_STATE polling + post-sync `loadHistory()` refresh so the operator gets closed-loop status without a second manual step. `renderPendingCard` now derives `reparsing` from `SYNC_STATE.status==='running' && p.retryQueued`; when true the row header pill flips to amber "Reparsing…" (overrides Needs Review; 260630-mav Missing Receipt pill preserved) and the Retry parse button renders in a disabled "Reparsing…" state (added `reparsing` OR into `showRetry` so the button doesn't vanish after the optimistic `parse_error=''` clear). Retry-parse click branch: idle-sync path POSTs `/retry-parse` then calls existing `triggerSync()`; running-sync path skips the POST (re-queue would be a no-op), just marks `row.retryQueued=true` and toasts "Reparse queued — sync already running"; 409 treated as success (re-queue idempotent per handler); non-2xx surfaces via `showToastMessage` instead of `alert()`. `retryQueued` is transient client-side state — cleared naturally when `refreshSyncStatus` fires `loadHistory()` on `running→done` transition (inventory.html:438). One targeted Playwright test in `tests/inventory.spec.js` asserts POST to `/retry-parse` + `/sync-receipts`, pill flip, button disabled state. Backend Go untouched (retry-parse handler at handler.go:872 unchanged). Frontend-only diff — version bump deferred to `/save-project`. | 2026-07-02 | 7d0a629 |  | [260702-l67-retry-parse-ux-auto-trigger-sync-on-clic](./quick/260702-l67-retry-parse-ux-auto-trigger-sync-on-clic/) |
