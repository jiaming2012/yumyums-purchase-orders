// tests/machine/reference-machine.mjs — the lockstep fuzz's behavioral
// reference (card redemption-submit-flow, run 20260905): the spike's
// hand-rolled parallel-region machine, copied VERBATIM from
// .night-crew/spikes/activity-c-scanner-screen/redemption-submit-flow/js/
// machine-handrolled.mjs. It passed the same 18-sequence suite and was proven
// observably equivalent to the overlay-region XState shape across 40,000
// lockstep walks in the spike — which is exactly what makes it the reference:
// the production strict machine must stay observably THIS machine, or the
// "which engine" choice has silently become a "which behavior" choice.
// TEST-ONLY: never shipped, never imported by page code.
//
