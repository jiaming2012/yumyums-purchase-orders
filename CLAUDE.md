...
- **Item aliases (nicknames):** Linking a receipt line to an item whose name differs auto-learns the raw receipt text as an alias (`POST /items/aliases`, upsert on `lower(alias)` — latest link wins; description always beats alias at match time). Managed as "Nicknames" chips in the Setup item editor; merges carry the source's aliases and learn its name.
