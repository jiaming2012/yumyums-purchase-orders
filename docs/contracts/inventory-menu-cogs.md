...
| `:255`–`:270` | Published `IngredientAlloc` / `UnallocatedBlock` / `UnallocatedDetail` client structs | **CONFIRMED byte-accurate** — **eight** json tags across three types (3 + 2 + 3), all matching `recipes.IngredientAlloc`, `recipes.UnallocatedBreakdown` and `recipes.UnallocatedDetail`. *(Corrected 2026-08-03 at morning triage: the second pass wrote "nine". Counted at source — `IngredientAlloc` carries `purchase_item_description`, `usage_pct`, `allocated_cost`; `UnallocatedBreakdown` carries `total`, `by_ingredient`; `UnallocatedDetail` carries `purchase_item_description`, `amount`, `reason`. The verdict — byte-accurate — is unaffected; only the count was wrong, and a wrong count in a row asserting accuracy is exactly the kind of thing this audit exists to catch in itself.)* **Added by the second pass**, which is worth stating plainly: the first pass audited `:243`–`:253` and stopped, so the three structs immediately beneath the one it condemned were never checked. They happen to be right. |
