...
| `tracked_bank_tx_ids` | array of strings | **NEVER PUBLISHED UNTIL 2026-08-03.** Added by `f730485` (2026-06-06). Every Mercury `bank_tx_id` HQ has touched for the period across all states — confirmed in `purchase_events`, or pending/confirmed/discarded in `pending_purchases`. **No `mercury_category` filter** — this list is for completeness detection, not COGS. Intended use: diff it against Mercury's own transaction list for the same period to find "Mercury has it, HQ has not ingested it yet" gaps. 🛑 **DO NOT ADD A CATEGORY FILTER TO THIS FIELD.** Confirmed 2026-08-04 by the consumer: it decodes this field and its Mercury↔HQ gap check depends on the list being unfiltered — that is what makes the check able to surface miscategorised and uncategorised rows, which are invisible to every COGS field on this endpoint. It is the consumer's stated backstop for exactly those rows. Filtering it would silently blind that check rather than break it. The consumer asked to be told if this ever changes; treat it as a breaking change even though it is only a narrowing. |
...
- [ ] **A5 (AMENDED AGAIN 2026-08-04 — it is NOT a coordinated two-repo release. HQ has BUILT this and NOT DEPLOYED it; `America/New_York` will be the operating timezone, `America/Chicago` in production today.)**

  🛑 **CORRECTED 2026-08-04 on the consumer's own reply: "sales-processor must make the matching change" was WRONG, and it had been gating the HQ deploy on nothing.** The consumer reports it has **no timezone code whatsoever** — it sends `from`/`to` as bare calendar dates and its Mercury gap check only diffs transaction IDs. The only change on that side is a **find-replace in its own handoff docs**, which it will do **after** migration `0072` deploys, deliberately not before.

  **What this means operationally:** the HQ deploy is **not blocked by the consumer**. The previous wording ("this is the one assumption that is not merely confirm-and-proceed; it requires a change on the sales-processor side") described a code dependency that does not exist. HQ still owes the consumer **the changeover date**, which is the only thing it is waiting on.

  ⚠️ **Provenance, stated because it differs from every other row on this page:** this correction comes from the **consumer's reported reply**, not from HQ inspecting the consumer's code — that repo is not present in this tree and HQ cannot verify it. Every other verdict in `§0` was checked at source. Treat this one as reported-and-credible rather than measured.
